...
このモジュールの使用は、Tealium Customer Data Hubを使用している場合に推奨されます。Customer Data Hubのユーザーインターフェースでデータソースキーを生成します。Tag Managementのみを使用している場合、現在は新しい変数を各ディスパッチに追加する以外の効果はありません。データソースについて[詳しく学ぶ]()。
...
フレームワークは自動的にインスタンス化されます。`TealiumCore` podに依存性があります。iOSのCocoaPodsインストールについて[詳しく学ぶ](/ja/platforms/ios-swift-v1/install/#cocoapods)。
...
フレームワークは自動的にインスタンス化されます。`TealiumCore`に依存性があります。追加のインポート文は必要ありません。iOSのCarthageインストールについて[詳しく学ぶ](/ja/platforms/ios-swift-v1/install/#carthage)。
...
## APIリファレンス
公開APIメソッドはありません。データソース変数はTealiumConfigオブジェクトを介して構成されます（[Swift Class: TealiumConfig](/ja/platforms/ios-swift-v1/api/tealium-config/)を参照）
