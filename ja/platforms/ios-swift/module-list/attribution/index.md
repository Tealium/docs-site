...
[iOS向けのSwift Package Managerのインストール](/ja/platforms/ios-swift/install/#swift-package-manager-recommended)について詳しく学びましょう。
...
[iOS向けのCocoaPodsのインストール](/ja/platforms/ios-swift/install/#cocoapods)について詳しく学びましょう。
...
[iOS向けのCarthageのインストール](/ja/platforms/ios-swift/install/#carthage)について詳しく学びましょう。

## 初期化

モジュールを初期化するには、[`collectors`](/ja/platforms/ios-swift/api/tealium-config/#collectors)プロパティに含めます。
...
`AppTrackingTransparency`と`SKAdNetwork`フレームワークについては、[Consent and the Apple IDFA (Identifier for Advertisers)](/ja/platforms/getting-started-mobile/idfa/)で詳しく学びましょう。
...
該当する場合は、`ATTrackingManager.requestTrackingAuthorization`を別途呼び出します。[Tealium SDKを有効にするためのトラッキング認証のリクエスト](/ja/platforms/getting-started-mobile/idfa/)について詳しく学びましょう。
...
[`SKAdNetwork`フレームワーク](/ja/platforms/getting-started-mobile/idfa/)について詳しく学びましょう。 &lt;!-- blog --&gt;
...
さらに、[`skAdConversionKeys`](/ja/platforms/ios-swift/api/tealium-config/#skadconversionkeys)を定義している場合、`SKAdNetwork.updateConversionValue(_:)`メソッドが呼び出され、指定されたコンバージョン値が渡されます。Tealiumは、既に設置しているトラッキングを通じて`SKAdNetwork`への呼び出しを管理することを可能にします。
