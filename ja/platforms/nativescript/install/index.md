...
アプリがインストールされた後、[TealiumConfig](/ja/platforms/nativescript/api/tealium-config/#tealiumconfig)初期化オプションを使用して`initialize()`メソッドで`Tealium`インスタンスを初期化します：  
...
詳細な情報と追加の構成オプションについては、[APIリファレンス](/ja/platforms/nativescript/api/)を参照してください。
...
これらのモジュールは、[`TealiumConfig`](/ja/platforms/nativescript/api/tealium-config/)の`collectors`プロパティを使用して有効または無効にします。
