...
| `consentLoggingEnabled`| `Boolean` | [同意ログ]()機能を有効にし、すべての同意ステータスの変更を監査目的でTealium Customer Data Hubに送信します。 (デフォルト: 有効) | `true` |
...
| `deepLinkTrackingEnabled` | `Boolean` | Facebookやその他のソースからアプリへのリンクなどの[標準的なディープリンクの自動トラッキング](/ja/platforms/getting-started-mobile/deep-linking/#readout)を有効または無効にします。また、QRトレースも有効または無効にします。 (デフォルト: 有効) | `false` |
...
| `qrTraceEnabled` | `Boolean` | [QRトレース](/ja/platforms/getting-started-mobile/trace/#mobile-trace-tool)を有効または無効にします。 (デフォルト: 有効) | `false` |
...
| `useRemoteLibrarySettings`| `Boolean` | [モバイル公開構成]()を有効または無効にします (デフォルト: 有効)。iQタグ管理でモバイル公開構成を構成するか、機能を無効にします。 | `false` |
| `visitorServiceEnabled`| `Boolean` | [データレイヤー拡張API]()を使用してビジタープロファイルの自動取得を有効または無効にします (デフォルト: 無効) | `true` |
...
これらのモジュールは、[`TealiumConfig`](/ja/platforms/nativescript/api/tealium-config/)の`collectors`プロパティを使用して有効または無効にできます。


### `ConsentPolicy`

遵守する同意ポリシーを定義します。[`TealiumConfig`](/ja/platforms/nativescript/api/tealium-config/)オブジェクトに同意ポリシーが定義されていない場合、同意マネージャーは無効になります。
