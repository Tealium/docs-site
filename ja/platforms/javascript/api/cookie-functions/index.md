...
| `session` | セッション終了時間で表す有効期限（デフォルトは[セッションタイムアウト](/ja/platforms/javascript/settings/#session-timeout)：30分）| `&#34;exp-session&#34;` |

タイムスタンプベースの有効期限（`&#34;exp-1549271521u&#34;`など）には、Unixタイムスタンプであることを示す`u`が末尾に付きます。Cookie自体で構成されている値（`&#34;exp-1549271521&#34;`など）と混同しないでください。
