...
* 同じサイトで複数のiQプロファイルを実行している場合、以下の手順を完了して、サイト上の`utag.js`のすべてのインスタンスが新しいクッキーの動作を同時に開始し、競合や互換性の問題を防ぎます：
    ...
    1. 各プロファイルで[`split_cookie`]()オーバーライドを`false`に構成して、過去のバージョンのutag.jsとの互換性を保つためにレガシークッキーの動作を使用します。
...
詳細については、[構成：`always_set_v_id`](/ja/platforms/javascript/settings/#always_set_v_id)をご覧ください。