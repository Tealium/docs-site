...
- 以下のいずれかのモバイルライブラリ：
  - [Android用Tealium](/ja/platforms/android-java/)
  - [iOS-Swift用Tealium](/ja/platforms/ios-swift/)
...
5. Tealium Swiftライブラリから追加のモジュールを使用している場合は、[Swiftパッケージマネージャーの手順](/ja/platforms/ios-swift/install/#swift-package-manager-recommended)に従って追加します。
...
3. その他の必要なモジュールを手動でPodfileに追加します。以下は例です：   
      ...
      ```
      iOS向けの[推奨モジュール](/ja/platforms/ios-swift/modules/)について詳しくは、ドキュメントを参照してください。
...
1. [Android用Tealium](/ja/platforms/android-java/install/)をインストールし、プロジェクトのトップレベルの`build.gradle`ファイルにTealium Maven URLを追加します。
...
エアシップリモートコマンドの手動インストールには、[Swift用Tealium](/ja/platforms/ios-swift/)ライブラリのインストールが必要です。iOSプロジェクトにエアシップリモートコマンドをインストールするには、次の手順を実行します。
...
エアシップリモートコマンドの手動インストールには、[Android（Kotlin）用Tealium](/ja/platforms/android-kotlin/install/)または[Android（Java）用Tealium](/ja/platforms/android-java/install/)のインストールが必要です。Androidプロジェクトにエアシップリモートコマンドをインストールするには、次の手順を実行します。
