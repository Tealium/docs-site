...
---
Tealiumのモバイルライブラリで[同意管理](/ja/platforms/getting-started-mobile/consent-management/)を有効にすると、ユーザーがトラッキングに同意するまで、デバイスからのトラッキングデータは送信されません。ユーザーの同意は、リモートコマンドと、アプリに統合されたサードパーティのベンダーSDKでも強制される必要があります。以下のシナリオでは、リモートコマンドでの同意の取り扱い方法を説明します。
...
一部のベンダーSDKでは、リモートコマンドがトリガーされる前にオプトインコマンドを呼び出す必要があります。具体的なコマンドについては、各[ベンダー統合](/ja/platforms/remote-commands/integrations/)を参照してください。
...
[同意カテゴリー](/ja/iq-tag-management/consent-management/consent-preferences-dialog/manage/)と[同意イベント仕様]()について詳しく学びましょう。
...
[JSON構成マッピング](/ja/platforms/remote-commands/vendor-integrations/#mappings)について詳しく学びましょう。
...
各ベンダーSDKは同意を異なる方法で管理します。各ベンダーSDKの同意オプションの構成方法についての詳細は、[リモートコマンド統合](/ja/platforms/remote-commands/integrations/)のベンダー固有のリモートコマンドドキュメンテーションを参照してください。
