...
コネクターマーケットプレイスにアクセスし、新しいコネクターを追加してください。コネクターを追加する一般的な手順については、[コネクターについて]()を参照してください。
...
* 私有鍵認証の場合：
    * **Private Key**: 顧客が生成した私有鍵。暗号化された私有鍵と暗号化されていない私有鍵の両方をサポートします。Snowflakeの私有鍵の生成方法については、[Snowflake: キーペア認証およびキーペアローテーション](https://docs.snowflake.com/en/user-guide/key-pair-auth#generate-the-private-key)を参照してください。私有鍵が暗号化されている場合は、Private Key Passphraseを提供する必要があります。
...
1. **Server-Side &gt; EventStream &gt; Event Connectors** または **Server-Side &gt; AudienceStream &gt; Audience Connectors** に移動します。
