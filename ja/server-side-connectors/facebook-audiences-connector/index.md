...
このコネクタは、ベンダーへの大量データ転送をサポートするためにバッチリクエストを使用します。並行処理により、イベントがベンダーに順不同で到達する可能性があります。順序が重要な場合は、イベントにシーケンス値を追加してください。詳細については、[バッチ処理アクション]()を参照してください。リクエストは、次のいずれかの閾値に達するか、プロファイルが公開されるまでキューに入れられます：
...
あなたのアカウントには複数の訪問ID属性が定義されている場合があります。この場合、[「既知の訪問」という訪問バッジ]()を作成して、いずれかの訪問IDの存在を確認することが重要です。この属性を使用することで、Facebookに渡すことができる訪問IDが割り当てられた訪問のみを含むオーディエンスを保証します。未知の訪問をターゲットにすることはできません。
...
最も効果的なテスト方法は、[トレース]()を実行してイベントが適切に処理されていることを確認し、Facebookダッシュボードをチェックしてカスタムオーディエンスが作成され、人が追加されていることを確認することです。
