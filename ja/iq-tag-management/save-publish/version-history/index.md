...
---
[プラットフォーム権限]()が有効になっているアカウントでは、クライアントサイドバージョンの権限を持つユーザーのみがこの画面にアクセスできます。詳細については、[権限グループ]()を参照してください。
...
バージョンタイトルの横にある下矢印をクリックして、次の操作を実行します：

![](/images/iq-tag-management/save-publish/versionhistory3.png)

* **このバージョンに切り替える**  
選択したバージョンを読み込みます。別のバージョンに切り替えると、ピンアイコンが表示されているバージョンのタイトルの隣に自動的に移動します。[アーカイブされたバージョン](#archived-versions)には切り替えることができません。
* **詳細の展開/非表示**  
...
* **このバージョン以降のメモを表示**  
選択したバージョンおよびその後の公開のメモを表示します。メモは公開および保存されたバージョンのメモが親バージョン内でインデントされる順に並べられます。

## 現在のセッションに変更をマージする

現在のセッションにマージ可能な変更がある場合、**現在のセッションに変更をマージする**オプションがバージョンアクションのリストとバージョン図の上に表示されます。

* **現在のセッションに変更をマージする**ドロップダウンリストから、このセッションに[マージしたいバージョン]()を選択します。受信バージョンと現在のバージョンは、同じ起源または祖先から派生している必要があります。
* ダッシュボードで**バージョン関係を表示する**フィルタを有効にすると、各バージョンが他のバージョンとどのように関連しているか、およびどのバージョンをマージできるか、またはできないかが表示されます。
...
1. **iQタグ管理 &gt; クライアントサイドバージョン**に移動します。  
...
1. 現在公開されているバージョンの横にある下矢印をクリックし、**詳細を展開**を選択します。
1. **Dev公開の詳細**の横にある下矢印をクリックして展開します。
1. **utag.jsを表示**をクリックします。  
...
* **拡張機能**：[拡張機能詳細ページ]()で**変更履歴を表示**をクリックします。
...
バージョン間の変更をマージすることで、古いバージョンから現在公開されているバージョンに変更を取り込むプロセスが簡素化されます。2つのバージョンをマージするには、同じ起源から分岐している必要があります。詳細については、[現在のセッションにマージする]()を参照してください。

![](/images/iq-tag-management/whiteui-tiq-versions-merge-into-current-session.png)

## アーカイブされたバージョン

古くて使用されていないバージョンは、サイトのパフォーマンスを向上させるためにアーカイブされます。アーカイブされたバージョンは、タイトルに**(アーカイブ済み)**と表示され、バージョン履歴に表示されます。アーカイブされたバージョンはロードできません。アーカイブされたバージョンを使用する必要がある場合は、Tealiumサポートに連絡してください。