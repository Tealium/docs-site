---
title: Tag Management
description: Tealium iQ タグ管理は、ページコードを編集することなくタグを管理するためのウェブインターフェースです。ウェブサイトのハードコードされたタグを単一のタグに置き換え、長い開発サイクルを必要とせずに簡単にタグを管理できます。
url: https://docs.tealium.com/ja/iq-tag-management/
---

![](/images/icons/icon-forward.svg)

#### はじめに

タグ管理が初めてですか？ここから始めて、その仕組みを学びましょう。




![](/images/icons/icon-code.svg)

#### コードを取得

サイトにインストールする準備ができたら、「環境管理」を訪れてください。



![](/images/icons/icon-badge-check.svg)

#### 保存と公開のベストプラクティス

プロファイルへの更新を開発、テスト、公開するためのベストプラクティス。



![](/images/icons/icon-cubes.svg)

#### データレイヤー

データレイヤーはデジタル戦略の基盤です。



![](/images/icons/icon-tag.svg)

#### タグ

タグ管理の基本を学びましょう。



![](/images/icons/icon-bolt.svg)

#### イベント

一般的な訪問のインタラクションを追跡するイベントを構成します。



![](/images/icons/icon-wrench.svg)

#### 拡張機能

データレイヤーを変更したり、イベント追跡を追加したりする必要がありますか？そのための拡張機能があります。



![](/images/icons/icon-tag.svg)

#### Moments iQ

訪問からリアルタイムで好みや感情（ゼロパーティデータ）を収集します。



![](/images/icons/icon-circles-overlap.svg)

#### ロードルール

タグをロードするタイミングと場所を定義します。



![](/images/icons/icon-shield.svg)

#### 同意管理

同意プロンプトを展開し、ユーザーの構成を追跡します。



![](/images/icons/icon-shield.svg)

#### 同意統合

Tealium以外の同意管理プラットフォームによってキャプチャされたエンドユーザーの同意決定を適用します。


## 保存と公開

協力的な開発環境でタグ構成の複数のバージョンを管理および公開します。


![](/images/icons/icon-arrow-right-into.svg)

保存と公開



![](/images/icons/icon-merge.svg)

クライアントサイドバージョン



![](/images/icons/icon-arrows-merge.svg)

バージョンのマージ


## 拡張機能

データレイヤーを変更したり、イベント追跡を追加したりする必要がありますか？そのための拡張機能があります！


![](/images/icons/icon-wrench.svg)

データ値の構成



![](/images/icons/icon-wrench.svg)

ルックアップテーブル



![](/images/icons/icon-wrench.svg)

高度なJavaScriptコード



![](/images/icons/icon-wrench.svg)

データ値の結合



![](/images/icons/icon-wrench.svg)

小文字化



![](/images/icons/icon-wrench.svg)

Eコマース


## 人気のタグ


![](/images/integrations/adobe-analytics.png)

Adobe Analytics



![](/images/integrations/bing-ads.png)

Bing Ads (UET)



![](/images/integrations/facebook.png)

Facebook Pixel



![](/images/integrations/floodlight.png)

Floodlight (gtag.js)



![](/images/integrations/google-ads-conversion.png)

Google Ads (gtag.js)



![](/images/integrations/google-analytics.png)

Google Analytics 4



![](/images/integrations/linkedin.png)

LinkedIn Insight Tag



![](/images/integrations/tiktok.png)

TikTok Pixel


## インストール


![](/images/icons/icon-globe.svg)

#### ウェブ用クイックスタート

Tealiumをサイトにインストールします。

はじめに



![](/images/icons/icon-mobile.svg)

#### モバイル用クイックスタート

Tealiumをモバイルアプリにインストールします。

はじめに


## 開発者ガイド


![](/images/icons/icon-platform-javascript-web.svg)

JavaScript (utag.js)



![](/images/icons/icon-platform-android.svg)

Tealium for Android



![](/images/icons/icon-platform-ios.svg)

Tealium for iOS
