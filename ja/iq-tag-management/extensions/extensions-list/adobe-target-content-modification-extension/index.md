...
* [utag.sync.js でのグローバル Mbox の使用]()

## 仕組み

Adobe Target コンテンツ変更拡張は、Adobe Target タグまたは Adobe Experience Platform Web SDK タグと一緒に使用して、HTML要素のコンテンツを変更します。これは、条件に基づいてA/Bコンテンツを表示するために使用されます。この拡張機能を動作させるためには、[Adobe Target タグ]()または [Adobe Experience Platform Web SDK タグ]()にスコープを構成する必要があります。
