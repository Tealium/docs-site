---
title: ユニバーサルタグデバッガー
description: ユニバーサルタグ（UTAG）デバッガーは、ユニバーサルデータオブジェクトとトラッキングコールの検証に使用されるツールです。
...
UTAGデバッガーをTealiumツールとしてインストールするには、まず[Tealium Toolsブラウザ拡張機能]()をインストールします。
...
UTAGデバッガーが開かれると、上部セクションにページ上の `utag.js` ファイルが検出され、次の情報が表示されます：
...
* **スコープ** - 表示するUDOデータのスコープを選択します。デフォルトは **ロードルール前**です。
    ツールに表示されるUDO変数は、**すべてのタグ - ロードルール前** および **すべてのタグ - ロードルール後** にスコープされた拡張機能の後にキャプチャされます。[操作の順序]()について詳しく学びましょう。
* **データの表示** – 表示する変数のタイプを選択します。UDO変数は常に表示されます。