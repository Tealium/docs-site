...
クラウド属性は構造と動作がイベント属性と同一ですが、[制限データ]()としてマークすることはできず、CloudStreamには[プリロードされた属性]()が含まれていません。

 プロファイルで500以上の属性が必要な場合は、カスタマーサクセスマネージャーに連絡してください。 
...
アクティベーションは、マーケティング、分析、広告プラットフォームへのセグメントの配信プロセスです。コネクタアクションまたは関数を通じてアクティベーションを構成できます。詳細については、 および  を参照してください。
