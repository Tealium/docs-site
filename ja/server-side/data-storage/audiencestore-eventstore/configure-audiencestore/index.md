...
Audienceコネクタを構成した後、**DataAccess &gt; AudienceStore**にアクセスして、JSONまたはCSVファイルを表示およびダウンロードします。各ファイルには訪問プロファイルデータ（訪問および訪問属性）が含まれています。また、サードパーティのツールを使用してファイルを表示することもできます。詳細については、[ファイルの表示]()を参照してください。