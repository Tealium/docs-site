...
url: https://docs.tealium.com/ja/server-side/predict/audiences/enrich-your-output-attribute/
---
モデルがデプロイされ、予測を行っている後、一般的な次のステップは、予測を使用して1つ以上のオーディエンスを作成することです。

デプロイされたすべてのモデルで、訪問スコープの出力属性を生成します。デフォルトでは出力属性は各訪問後に持続しないため、この属性を訪問スコープにすることを検討するかもしれません。訪問スコープの出力属性を使用すると、[Visitor Search](https://docs.tealium.com/visitor-search/)や[Audience Discovery](https://docs.tealium.com/about-audience-discovery/)などの追加ツールを使用できます。

Tealium Predictによって生成された出力属性をエンリッチするには、次の手順を完了します：

1. **Transform > Predict > Deployed Model Dashboard**に移動します。
