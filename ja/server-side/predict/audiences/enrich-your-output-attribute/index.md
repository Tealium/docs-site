...
デプロイされたすべてのモデルで、訪問スコープの出力属性を生成します。デフォルトでは出力属性は各訪問後に持続しないため、この属性を訪問スコープにすることを検討するかもしれません。訪問スコープの出力属性を使用すると、[Visitor Search]()や[Audience Discovery]()などの追加ツールを使用できます。
