...
サービスの構成を完了した後、**データソースダッシュボード**でデータソースを展開し、**サービス構成**タブをクリックすることで、Tealium S3の資格情報を見つけることができます。
 ...
 これらの資格情報すべてが必要となりますので、Cyberduckを使用してバケットに接続します。Cyberduckを使用してFTP/SFTPまたはAmazon S3経由でファイルをアップロードする方法については、[Use Cyberduck to upload a file via FTP/SFTP or Amazon S3]()を参照してください。
