...
監視されたコネクターアクションがエラーを返すと、Tealiumは構造化されたJSONログレコードを生成し、ログソースを通じて関連する宛先コネクターに転送します。宛先プラットフォームはこれらのレコードをインデックス化し、ダッシュボード、アラート、および分析のために使用します。
...
* イベント属性を宛先が要求するパラメータにマッピングします。

各ログソースは次のように範囲を構成できます：
...
宛先の**ログソース**テーブルで任意のコネクターエラーログソースをクリックすると、その詳細が表示されます：
...
**コネクター**タブには、ログソースによって監視されているコネクターがリストされており、以下を含みます：
...
次の表は、コネクターエラーログで利用可能なすべてのパラメータをリストしています。マッピングでこれらのパラメータを参照する場合は、二重の中括弧を使用します（例：`{{connectorType}}`）。
...
| `customId` | コネクターアクションID。この値を見つけるには、コネクターをクリックし、次にアクションをクリックし、その後で**詳細**をクリックします。 |
