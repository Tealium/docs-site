...
各送信先には独自の構成要件があります。必要なフィールドとサポートされているパラメータについては、コネクタのドキュメントを参照してください。利用可能なコネクタのリストについては、[Available destination connectors]()を参照してください。
...
1. **New Log Source**ダイアログで：
   ...
   1. **Log Streaming Scope**を選択します：
      ...
      * **Send Log Event**：ペイロードのマップされた属性のみを送信します。
      ...
      利用可能なオプションが一つのみの場合、それがデフォルトで選択されます。詳細については、[Available destination connectors]()を参照してください。
...
利用可能なベンダーパラメータについての情報は、[destination connector and action]()および[connector log parameters]()のドキュメントを参照してください。
...
1. **イベント属性をエンリッチメントを使用して作成する**
   ...
   * 値として`{{severityText}}`を持つ文字列構成エンリッチメントを追加します。
   ...
   利用可能なログパラメータの完全なリストについては、[Connector log parameters]()を参照してください。
...
完全なパラメータ定義については、[Connector log parameters]()を参照してください。
