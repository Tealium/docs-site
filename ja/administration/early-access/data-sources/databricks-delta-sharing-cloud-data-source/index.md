...
[CloudStream]() と一緒に使用すると、データパイプラインはゼロコピーです。データはあなたの Databricks 環境内に留まり、オンデマンドでアクティベーションのためにアクセスされます。
...
1. Tealium サポートが接続を作成した後、Tealium で Databricks Delta Sharing クラウドデータソースを作成します。詳細については、を参照してください。
...
あなたの Databricks ワークスペースが IP アドレスによって制限されている場合、[Tealium IP アドレス]() を Databricks の IP アクセスリストに追加してください。
