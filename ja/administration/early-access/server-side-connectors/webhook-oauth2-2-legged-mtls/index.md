---
title: Webhook OAuth2 2-legged mTLSコネクタ
...
<blockquote>
Webhook OAuth2 2-legged mTLSコネクタは現在アーリーアクセス中で、選ばれた顧客のみが利用可能です。開始するにはTealiumサポート担当者に連絡してください。
</blockquote>
...
1. サーバーに証明書を信頼させるために以下のいずれかを構成します：
...
1. Tealiumは証明書を提示し、関連する秘密鍵の所有を証明します。
...
<blockquote>
証明書と秘密鍵はプロファイルに保存され、バックアップコピーはありません。証明書を生成した後にプロファイルを保存して公開しない場合、証明書は保存されず、アクセスを回復するためにTealiumサポートに連絡する必要があります。
</blockquote>
