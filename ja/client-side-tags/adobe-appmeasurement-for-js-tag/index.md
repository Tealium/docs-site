...
タグマーケットプレイスにアクセスして、Adobe Analytics AppMeasurement for JSタグを追加します（[タグの追加方法についてはこちら](https://docs.tealium.com/manage-tags/)を参照）。
...
* **デバイスタイプ**
  * 実装のプラットフォームに基づいて以下から選択してください：
    ...
    * **モバイルアプリ** [Tealium for iOS](https://docs.tealium.com/ja/platforms/ios-swift/) や [Tealium for Android](https://docs.tealium.com/ja/platforms/android-kotlin/) でのネイティブモバイルインストール用。
* **レポートスイート**
...
* **自動リンクトラッキング**
  ...
  * **いいえ**に構成する場合は、[リンクトラッキング拡張](https://docs.tealium.com/link-tracking-extension/)などの他の方法を使用して自動リンクトラッキングを再現する必要があります。
* **内部リンクフィルター**
...
マッピングは、[データレイヤー変数](https://docs.tealium.com/about-the-data-layer/)からベンダータグの対応する宛先変数にデータを送信するプロセスです。タグ宛先に変数をマッピングする方法については、[データマッピング](https://docs.tealium.com/ja/iq-tag-management/data-mappings/manage/)を参照してください。
...
AppMeasurementタグはeコマース対応であり、デフォルトで[E-Commerce Extension](https://docs.tealium.com/e-commerce-extension/)のマッピングを自動的に使用します。拡張マッピングをオーバーライドするか、拡張機能で提供されていない特定のeコマース変数を使用したい場合を除き、このカテゴリでの手動マッピングは通常必要ありません。
...
1. [データ値の構成](https://docs.tealium.com/set-data-values-extension/)拡張機能を追加します。
...
<blockquote>
Marketing Cloud ID Service Tagは、他のAdobeタグ（AppMeasurementを含む）よりも先にプロファイルで読み込む必要があります。そうしないと、訪問トラッキングの整合性が失われます。タグを最初にロードするには、[bundle setting](https://docs.tealium.com/tag-advanced-settings/)をAll Pagesで有効にし、他のAdobeタグよりも先にロードします。
</blockquote>
