...
新しいタグを追加するために、タグマーケットプレイスに移動します。タグを追加する方法の一般的な手順については、[タグの概要]() 記事を参照してください。
...
タグをすべてのページで読み込むか、タグが読み込まれる条件を構成します。ロードルールについての詳細は、[ロードルール]() のドキュメントを参照してください。

## データマッピング

マッピングとは、データレイヤー変数からベンダータグの対応する変数にデータを送信するプロセスです。変数をタグの宛先にマッピングする方法についての手順は、[データマッピング](/ja/iq-tag-management/data-mappings/manage/) を参照してください。
