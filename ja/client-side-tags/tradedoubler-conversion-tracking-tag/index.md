...
[ロードルール]()は、このタグのインスタンスをいつ、どこでロードするかを決定します。**すべてのページでロード**ルールがデフォルトのルールです。特定のページでこのタグをロードしたい場合は、関連する条件を持つ新しいルールを作成します。
...
Tradedoubler コンバージョン トラッキング タグは Eコマース対応であり、デフォルトの [Eコマース拡張機能]() マッピングを自動的に使用します。拡張機能のマッピングを上書きしたい場合や、拡張機能で提供されていない Eコマース変数が必要な場合を除き、このカテゴリでの手動マッピングは通常必要ありません。
