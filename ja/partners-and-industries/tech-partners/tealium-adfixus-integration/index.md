...
1. iQタグ管理インスタンスでAdFixus [データレイヤー変数](https://docs.tealium.com/manage-variables/#add-a-variable)を作成します。
    * AdFixus IDを使用するすべてのTealiumアカウントプロファイルでこの変数を作成します。
    * お使いのウェブサイトのクッキーにAdFixus IDが保存される可能性があります。詳細については、[AdFixus実装チーム](https://www.adfixus.com/contact)にお問い合わせください。![](https://docs.tealium.com/images/tech-partners/adfixus-add-variable.png)
1. AdFixus ID変数を送信する必要がある関連タグを特定し、[データポイントをマッピング](https://docs.tealium.com/manage-data-mappings/)します。
    1. **変数**ドロップダウンリストからデータ変数を選択し、**選択先を選択**をクリックします。![](https://docs.tealium.com/images/tech-partners/adfixus-select-destination.png)
    1. **カスタム選択先を追加**をクリックし、**カスタム選択先**フィールドにAdFixus IDの予想される名前を入力し、**+追加**をクリックします。![](https://docs.tealium.com/images/tech-partners/adfixus-custom-destination.png)
...
1. iQタグ管理も使用している場合は、[Tealium Collectタグ](https://docs.tealium.com/tealium-collect-tag/)を構成します。
  * AdFixus IDは、[イベント属性](https://docs.tealium.com/about-attributes/)の下でユニバーサル変数として表示されます。
1. iQタグ管理を使用していない場合は、AdFixus IDの[イベント属性を作成](https://docs.tealium.com/manage-es-attributes/#add-an-attribute)します。
    1. **イベント属性**に移動し、**+ 属性を追加**をクリックします。
    1. **ユニバーサル変数**を選択し、**続行**をクリックします。
    1. **文字列**を選択し、**続行**をクリックします。
    1. 属性名とその他の関連情報を入力します。![](https://docs.tealium.com/images/tech-partners/adfixus-add-attribute.png)
    1. **完了**をクリックします。
        * AdFixusからTealiumにさらにデータを送信する予定がある場合は、そのデータの属性も作成します。
        * データセットのすべての属性に対して[イベント仕様](https://docs.tealium.com/manage-event-specifications/)を作成することをお勧めします。
1. AdFixus IDの[訪問属性を作成](https://docs.tealium.com/manage-as-attributes/#add-an-attribute)します。
    ...
    1. 属性が`割り当てられている`かつ`none（大文字小文字を区別しない）と等しくない`場合のルールを作成します。![](https://docs.tealium.com/images/tech-partners/adfixus-attribute-ready.png)
1. 保存して公開します。
