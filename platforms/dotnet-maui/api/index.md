...
Sets a callback function to execute when the visitor profile has been updated. The updated [`VisitorProfile`](#instance-ivisitorprofile) is provided in the callback response. For use when [`visitorServiceEnabled`](https://docs.tealium.com/platforms/dotnet-maui/install/#config) is set to `true`.
