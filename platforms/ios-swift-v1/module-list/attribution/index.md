...
<blockquote>
The variables prefixed with `ad_` are only enabled if Search Ads has been explicitly enabled in the [TealiumConfig](https://docs.tealium.com/platforms/ios-swift-v1/api/tealium-config/) object. These variables are only retrieved from Apple's servers once during the app's lifetime, but they are stored as persistent variables so they are available on future app launches.
</blockquote>
