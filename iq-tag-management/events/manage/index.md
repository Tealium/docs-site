...
1. Under **Scope**, select a scope from the list to determine when the event listener initialization code runs on the page. We recommend that you use the **DOM Ready** scope. For more information, see [Event score](https://docs.tealium.com/event-scope/).
...
1. Under **Publish Locations**, select the publish locations for the event. If you deselect the checkbox for a publish location, the event listener will not be published to that location. For example, if you deselect **Prod**, the event listener will not be pushed to your live site, even if you select **Prod** as a publish target from the **Save** dialog. For more information, see [About publishing](https://docs.tealium.com/about-publishing/).
1. In the **Element Selector** box, enter the element ID, class, or selector of the item you want to target. For more information, see [Event element selector](https://docs.tealium.com/event-element-selector/).
1. Under **Event Triggers**, select the event triggers for this event and set the thresholds for those triggers. For more information about each event type's event triggers, read the documentation from our list of [Event Types](https://docs.tealium.com/event-types/).
1. Under **Trigger Frequency**, select whether you want to track every time that event listener is triggered on a page (**Always**) or track only the first time an event is triggered (**Once**). For more information, see [Event triggers](https://docs.tealium.com/event-triggers/).
1. Configure the trigger variables for the event listener. These are the values sent with the event listener tracking call. You can edit existing trigger variables or add new ones to collect additional data with the visitor actions. For more information about variables, see [Event Variables](https://docs.tealium.com/event-variables/). For more information about event type's event trigger variables, read the documentation from our list of [Event Types](https://docs.tealium.com/event-types/). 
1. When you are done configuring the event listener, click **Next**. 

### Step 3: Create rules

1. In the **Rules** dialog, set the rules that determine if a page loads your new event listener. The left side of the dialog displays the logic the page uses for loading the event listener. The right side of the dialog lists all the available rules on the profile that you can reuse for this event listener. ![](https://docs.tealium.com/images/guides/iq/event-rules.png)
1. To create a new rule, click **+ Add Rule**. For information about how to create and configure a rule, see [Load Rules](https://docs.tealium.com/about-load-rules/).
1. Build the logical conditions for the event listener by dragging and dropping the rules from the list on the right side of the dialog to the logic boxes on the left side. For more information about logical operators and load rules with events, see [Event Rules Examples](https://docs.tealium.com/event-rules/). 
<blockquote>
...
For more about tags and event listeners, see [Subscribe a tag to an event](https://docs.tealium.com/subscribe-tag-to-event/).
...
For more information about subscribing tags to event listeners, see [Subscribing Tags to Events](https://docs.tealium.com/subscribe-tag-to-event/).
